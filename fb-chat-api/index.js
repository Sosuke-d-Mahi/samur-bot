...
function loginHelper(appState, email, password, globalOptions, callback) {
	...
	if (appState) {
		...
		if (email) {
			throw { error: "Currently, the login method by email and password is no longer supported, please use the login kuki 🐢" };
		}
		else {
			throw { error: "kuki has skill isu or not provided 🦊" };
		}
	}
